...
class tracker:
    ...
    def find_window_centroids(self, warped):
        ...
        l_sum = np.sum(
            warped[int(3 * warped.shape[0] / 4) :, : int(warped.shape[1] / 2)], axis=0
        )
        ...
        r_center = (
            np.argmax(np.convolve(window, r_sum))
            - window_width / 2
        ...
        for level in range(1, (int)(warped.shape[0] / window_height)):
            ...
            image_layer = np.sum(
                warped[
                    int(warped.shape[0] - (level + 1) * window_height) : int(
                        warped.shape[0] - level * window_height
                    ),
                    :,
                ],
                axis=0,
