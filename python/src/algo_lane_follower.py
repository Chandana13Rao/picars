"""
Lane follower based on algorithms without using Machine Learning
"""
import pickle
...
def window_mask(width, height, img_ref, center, level):
    ...
    output[
        int(img_ref.shape[0] - (level + 1) * height) : int(
            img_ref.shape[0] - level * height
        ),
...
def process_image(img):
    ...
    with open("../assests/calibration_pickle.p", "rb") as file:
    ...
    for level in range(0, len(window_centroids)):
        ...
        l_mask = window_mask(
            window_width, window_height, warped, window_centroids[level][0], level
        )
        r_mask = window_mask(
            window_width, window_height, warped, window_centroids[level][1], level
        )
    ...
    warpage = np.array(
        cv2.merge((warped, warped, warped)), np.uint8
    )  # making the original road pixels 3 color channels
    result = cv2.addWeighted(
        warpage, 1, template, 0.5, 0.0
    ...
    left_lane = np.array(
        list(
            zip(
                np.concatenate(
                    (left_fitx - window_width / 2, left_fitx[::-1] + window_width / 2),
                    axis=0,
                ),
    ...
    inner_lane = np.array(
        list(
            zip(
                np.concatenate(
                    (left_fitx + window_width / 2, right_fitx[::-1] - window_width / 2),
                    axis=0,
                ),
                np.concatenate((yvals, yvals[::-1]), axis=0),
            )
    ...
    road1 = np.copy(road)
    cv2.putText(
        road1, "Polynomial fit", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2
    )
    ...
    curve_fit_cr = np.polyfit(
        ...
        np.array(leftx, np.float32) * xm_per_pix,
        2,
    )
    curverad = (
        (1 + (2 * curve_fit_cr[0] * yvals[-1] * ym_per_pix + curve_fit_cr[1]) ** 2)
        ** 1.5
    ...
    # draw the text showing curverad, offset & speed
    cv2.putText(
        result,
